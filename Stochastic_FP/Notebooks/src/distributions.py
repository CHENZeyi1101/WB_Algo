...
class DatasetSampler(Sampler):
    def __init__(self, dataset, num_workers=40, device='cpu'):
...
class SwissRollSampler(Sampler):
    def __init__(
        self, estimate_size=100000, device='cpu'
    ):
...
class NormalSampler(Sampler):
    def __init__(
        self, mean, cov=None, weight=None, device='cpu'
    ):
...
class CubeUniformSampler(Sampler):
    def __init__(
        self, dim=1, centered=False, normalized=False, device='cpu'
    ):

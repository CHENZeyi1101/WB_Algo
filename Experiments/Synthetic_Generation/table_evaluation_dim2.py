...
if __name__ == "__main__":
    '''
    Experiment~1: 2d synthetic generation 
    '''
    ...
    WIN_Korotin_dir = f"{all_outputs_dir}/WIN_Korotin_outputs"
    CWB_Li_dir = f"{all_outputs_dir}/CWB_Li_outputs"
    Fast_Cuturi_dir = f"{all_outputs_dir}/Fast_Cuturi_outputs"
    WDHA_Kim_dir = f"{all_outputs_dir}/WDHA_Kim_outputs"
    ...
    V_value_WIN_Korotin_path = f"{WIN_Korotin_dir}/evaluation_results/V_values/V_values_iter100200.json"
    V_value_CWB_li_path = f"{CWB_Li_dir}/V_values/V_values.json"
    V_value_Fast_Cuturi_path = f"{Fast_Cuturi_dir}/V_values/V_values.json"
    V_value_WDHA_Kim_path = f"{WDHA_Kim_dir}/V_values/V_values.json"
    ...
    assert os.path.exists(V_value_WDHA_Kim_path), f"File {V_value_WDHA_Kim_path} does not exist."
    ...
    W2_to_bary_WIN_Korotin_path = f"{WIN_Korotin_dir}/evaluation_results/W2_to_bary/W2_to_bary_iter100200.json"
    ...
    W2_to_bary_WDHA_Kim_path = f"{WDHA_Kim_dir}/W2_to_bary/W2_to_bary.json"
    ...
    assert os.path.exists(W2_to_bary_WDHA_Kim_path), f"File {W2_to_bary_WDHA_Kim_path} does not exist."
    ...
    with open(V_value_WDHA_Kim_path, "r") as f:
        V_value_WDHA_Kim = json.load(f)
    ...
    with open(W2_to_bary_WDHA_Kim_path, "r") as f:    
        W2_to_bary_WDHA_Kim = json.load(f)
    ...
    export_latex_table(
        results,
        output_path=f"{all_outputs_dir}/2d_results_table.txt",
        metric_names=[
            r"$V(\widehat{\mu})$)",
            r"$\mathcal{W}_2(\widehat{\mu}, \bar{\mu})$"],
        caption="Performances of algorithms in Experiment [SG-2d]",
        label="tab:results"
    )
...
if __name__ == "__main__":
    ...
    json_filename_fulldata = 'data_full.json'

    # Train on full dataset
    data_full = {'x': X.tolist(), 'y': Y.astype(int).tolist(), 'd': d, 'n': N, 'n_rep': 1}

    with open(json_filename_fulldata, 'w') as json_file:
    ...
    for i, part in enumerate(splits):
        ...
        json_filename_subset = f'data_split_{i}.json'

        with open(json_filename_subset, 'w') as json_file:
    ...
    # sample_from_meta(os.path.join(OUTPUT_MODEL_DIR, "model_total.meta.pkl"), num_chains=1, num_samples=10000, save_samples=True, save_dir=OUTPUT_SAMPLES_DIR) 
    ...
    #     sample_from_meta(os.path.join(OUTPUT_MODEL_DIR, f"model_split_{i}.meta.pkl"), num_chains=1, num_samples=10000, save_samples=True, save_dir=OUTPUT_SAMPLES_DIR)
